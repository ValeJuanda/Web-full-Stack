...
const obtener = async (req, res) => {
    try {
        ...
        if (!estudiante) return res.status(404).json({ success: false, data: null, message: 'Estudiante no encontrado' });
...
const crear = async (req, res) => {
    ...
    if (!nombre || !apellido || !dni) return res.status(400).json({ success: false, data: null, message: 'Nombre, apellido y DNI son obligatorios' });
    try {
...
const actualizar = async (req, res) => {
    try {
        ...
        if (filas === 0) return res.status(404).json({ success: false, data: null, message: 'Estudiante no encontrado' });
